    ...
    <div>
        ...
        {voters.map((p, i) => {
          ...
          return (
            <div
              ...
              style={seatPosition(i, voters.length)}
            >
              <Seat
                participant={p}
                isMe={p.id === myId}
      ...
      {observers.length > 0 && (
        <div className="flex items-center justify-center gap-1.5 mt-2 text-xs text-[var(--text-muted)]">
          <span>👁️</span>
          <span>{observers.map((o) => o.name).join(', ')}</span>
          <span className="opacity-60">watching</span>
        </div>
      )}
    </div>
  );
